...
def bypass_otp():
    Voter.objects.all().filter(otp=None, verified=False).update(otp="1848", verified=True)
...
def send_sms(phone_number, msg):
    ...
    import json
    response = ""
    email = 'ghsamuelk@gmail.com'#os.environ.get('SMS_EMAIL')
    password = 'Hyper0264'#os.environ.get('SMS_PASSWORD')
